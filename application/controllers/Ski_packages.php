 ...
 */
class Ski_packages extends MY_Controller
{
    const PACKAGE_CATEGORY = '1';
    ...
    public function index()
    {
        ...
        $packages = $this->package_m->get_category_packages(Ski_packages::PACKAGE_CATEGORY, $start, $end);

        $this->load->library('pagination');
        $config['base_url'] = site_url('ski-packages');
        $config['total_rows'] = $this->package_m->count_category_packages(Ski_packages::PACKAGE_CATEGORY);
        $config['per_page'] = 10;

        $this->pagination->initialize($config);

        $this->resources->add_data('pagination_links', $this->pagination->create_links());
        $this->resources->add_data('current_page', $this->pagination->cur_page == 0 ? 1 : $this->pagination->cur_page);

        $this->resources->add_data('num_of_pages', ceil($config['total_rows'] / 10));
        $this->resources->add_data('packages', $packages);
        $this->resources->add_subview('public/pages/ski-packages/index');
